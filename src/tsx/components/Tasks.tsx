	...
	return (
		<div className={Style.container}>
			<div className={Style.task}>
				...
				<p className={Style.task__text}>{Task.title}(id: {id})</p>
				<h3 className={Style.task__title}>タスクの詳細</h3>
				<p className={Style.task__text}>{Task.detail}</p>
				<h3 className={Style.task__title}>ステータス</h3>
				<p className={Style.task__text}>{StatusList.filter((status) => status.id === Task.status)[0].label}</p>
				<h3 className={Style.task__title}>カテゴリー</h3>
				<p className={Style.task__text}>{Task.category}</p>
